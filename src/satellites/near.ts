...
async function authenticateNearSignature(
...
export const verifyNearSig: MethodFn = async ({ metadata, inputs }) => {
  const network = metadata["network"];

  if (!["near", "*"].includes(network)) {
    throw new Error("invalid network for the orb method");
  }

  const message = inputs[0].toString();
  const accountId = inputs[1].toString();
  const publicKey = inputs[2].toString();
  const nonce = inputs[3].toString();
  const signature = inputs[4].toString();

  const isValid: boolean = (
    await authenticateNearSignature(
      message,
      accountId,
      publicKey,
      nonce,
